...
export default function SignInWorker() {
    ...
    const getEmployee = () => {
        const apiUrl = 'http://localhost:57268/api/Employees/' + inputNumber;
        fetch(apiUrl, {
            method: 'GET',
            headers: new Headers({
            ...
            .then(
                (result) => {
                    ...
                    setEmployee({ Id: result.Id, num: result.EmployeeNumber });
    ...
    const check = () => {
        getEmployee();
        if (employee.Id === inputId) { navigate('/HomeManager') };
